...
describe("/POST /milks", () => {
  ...
  describe("/POST /milks - failed", () => {
    it("should failed create milk - totalBags min. 1", async () => {
      ...
      const response = await request(app).post("/milks").set("Authorization", `Bearer ${access_token_user}`).send(body)
    ...
    it("should failed create milk - invalid token", async () => {
      const response = await request(app).post("/milks").set("Authorization", `Bearer asjdkasjdadjskl`)
...
describe("/GET /milks", () => {
  describe("/GET /milks - succeed", () => {
    it("success get milks", async () => {
      const response = await request(app).get("/milks").set("Authorization", `Bearer ${access_token_user}`)
    ...
    it("success get milks", async () => {
      const location = "3674"
      const response = await request(app).get(`/milks?location=${location}`).set("Authorization", `Bearer ${access_token_user}`)
    ...
    it("success get milks", async () => {
      ...
      const response = await request(app).get(`/milks?compability=${compability}`).set("Authorization", `Bearer ${access_token_user}`)
    ...
    it("success get milks", async () => {
      const location = "3674"
      const compability = "asc"
      const response = await request(app).get(`/milks?compability=${compability}&location=${location}`).set("Authorization", `Bearer ${access_token_user}`)
  ...
  describe("/GET /milks - failed", () => {
    it("failed get milks - invalid token", async () => {
      const response = await request(app).get("/milks").set("Authorization", `Bearer salahhh`)
...
describe("/DELETE /milks", () => {
  describe("/DELETE /milks - succeed", () => {
    it("success delete milk", async () => {
      ...
      const response = await request(app).delete(`/milks/${params}`).set("Authorization", `Bearer ${access_token_user}`)
